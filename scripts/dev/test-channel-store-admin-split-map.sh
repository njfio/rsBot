...
SPLIT_MAP_SCRIPT="${SCRIPT_DIR}/channel-store-admin-split-map.sh"
...
source_file="${tmp_dir}/channel_store_admin.rs"
missing_source_file="${tmp_dir}/missing.rs"
output_json="${tmp_dir}/split-map.json"
output_md="${tmp_dir}/split-map.md"

cat >"${source_file}" <<'EOF'
pub fn execute_channel_store_admin_command() {}
fn collect_operator_control_summary_report() {}
...
assert_contains "$(cat "${output_md}")" "Channel Store Admin Split Map" "functional markdown title"
...
echo "channel-store-admin-split-map tests passed"
