...
SPLIT_MAP_SCRIPT = REPO_ROOT / "scripts" / "dev" / "channel-store-admin-split-map.sh"
SCHEMA_PATH = REPO_ROOT / "tasks" / "schemas" / "m25-channel-store-admin-split-map.schema.json"
GUIDE_PATH = REPO_ROOT / "docs" / "guides" / "channel-store-admin-split-map.md"
REPORT_JSON_PATH = REPO_ROOT / "tasks" / "reports" / "m25-channel-store-admin-split-map.json"
REPORT_MD_PATH = REPO_ROOT / "tasks" / "reports" / "m25-channel-store-admin-split-map.md"

REQUIRED_GUIDE_SNIPPETS = (
    "channel-store-admin-split-map.sh",
    "m25-channel-store-admin-split-map.schema.json",
    "m25-channel-store-admin-split-map.json",
    "m25-channel-store-admin-split-map.md",
    "Public API Impact",
    "Test Migration Plan",
)


class ChannelStoreAdminSplitMapContractTests(unittest.TestCase):
    ...
    def test_regression_report_matches_schema_version_and_target_budget(self):
        ...
        self.assertEqual(payload["source_file"], "crates/tau-ops/src/channel_store_admin.rs")
        self.assertEqual(payload["target_line_budget"], 2200)
