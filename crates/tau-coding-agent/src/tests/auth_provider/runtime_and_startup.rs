...
async fn regression_spec_2542_c03_run_local_runtime_prompt_executes_model_call() {
    ...
    cli.prompt = Some("mutation guard prompt".to_string());
    ...
    let recorded_models = Arc::new(AsyncMutex::new(Vec::new()));
    let client: Arc<dyn tau_ai::LlmClient> = Arc::new(RecordingSequenceClient {
        outcomes: AsyncMutex::new(VecDeque::from([Ok(ChatResponse {
            message: Message::assistant_text("runtime-ok"),
            finish_reason: Some("stop".to_string()),
            usage: ChatUsage::default(),
        })])),
        recorded_models: Arc::clone(&recorded_models),
    });
