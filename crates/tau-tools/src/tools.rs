...
fn optional_string_array_unbounded(arguments: &Value, key: &str) -> Result<Vec<String>, String> {
    let Some(value) = arguments.get(key) else {
        return Ok(Vec::new());
    };
    let Some(items) = value.as_array() else {
...
fn optional_string_map(arguments: &Value, key: &str) -> Result<BTreeMap<String, String>, String> {
    let Some(value) = arguments.get(key) else {
...
fn resolve_background_job_runtime(
    policy: &Arc<ToolPolicy>,
...
fn background_job_record_payload(
...
fn background_job_health_payload(health: &tau_runtime::BackgroundJobHealthSnapshot) -> Value {
    json!({
...
fn read_output_preview(path: &Path, max_bytes: usize) -> Result<String, String> {
    if !path.exists() {
