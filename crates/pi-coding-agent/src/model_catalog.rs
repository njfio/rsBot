...
pub(crate) const MODELS_LIST_USAGE: &str = "/models-list [query] [--provider <name>] [--tools <true|false>] [--multimodal <true|false>] [--reasoning <true|false>] [--limit <n>]";
pub(crate) const MODEL_SHOW_USAGE: &str = "/model-show <provider/model>";
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub(crate) struct ModelCatalogLoadOptions {
    pub(crate) cache_path: PathBuf,
    pub(crate) refresh_url: Option<String>,
    pub(crate) offline: bool,
    pub(crate) stale_after_hours: u64,
...
impl ModelCatalog {
    ...
    pub(crate) fn entries(&self) -> &[ModelCatalogEntry] {
        &self.entries
    }

    pub(crate) fn source(&self) -> &ModelCatalogSource {
        &self.source
    }

    pub(crate) fn find(&self, provider: &str, model: &str) -> Option<&ModelCatalogEntry> {
    ...
    pub(crate) fn find_model_ref(&self, model_ref: &ModelRef) -> Option<&ModelCatalogEntry> {
        self.find(model_ref.provider.as_str(), &model_ref.model)
    }

    pub(crate) fn is_stale(&self, stale_after_hours: u64) -> bool {
    ...
    pub(crate) fn diagnostics_line(&self, stale_after_hours: u64) -> String {
...
#[cfg(test)]
pub(crate) fn default_model_catalog_cache_path() -> PathBuf {
    PathBuf::from(".pi/models/catalog.json")
}

pub(crate) fn parse_model_catalog_payload(payload: &str) -> Result<ModelCatalogFile> {
...
pub(crate) fn validate_model_catalog_file(file: &ModelCatalogFile) -> Result<()> {
...
pub(crate) async fn load_model_catalog_with_cache(
...
pub(crate) fn ensure_model_supports_tools(
...
pub(crate) fn parse_models_list_args(input: &str) -> Result<ModelListArgs> {
...
pub(crate) fn render_models_list(catalog: &ModelCatalog, args: &ModelListArgs) -> String {
...
pub(crate) fn render_model_show(catalog: &ModelCatalog, raw_model: &str) -> Result<String> {
    ...
    fn regression_model_catalog_diagnostics_reports_stale_cache_age() {
        let catalog = ModelCatalog {
            ...
            source: ModelCatalogSource::Cache {
                path: PathBuf::from(".pi/models/catalog.json"),
            },
