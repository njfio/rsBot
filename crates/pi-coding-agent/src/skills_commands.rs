...
pub(crate) fn parse_trust_rotation_spec(raw: &str) -> Result<(String, TrustedKey)> {
...
pub(crate) fn load_trust_root_records(path: &Path) -> Result<Vec<TrustedRootRecord>> {
    if !path.exists() {
        return Ok(Vec::new());
    }
...
pub(crate) fn save_trust_root_records(path: &Path, records: &[TrustedRootRecord]) -> Result<()> {
