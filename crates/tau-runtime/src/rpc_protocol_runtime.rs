...
pub fn parse_rpc_frame(raw: &str) -> Result<RpcFrame> {
    let frame =
...
pub fn validate_rpc_frame_file(path: &Path) -> Result<RpcFrame> {
    let raw = std::fs::read_to_string(path)
        .with_context(|| format!("failed to read rpc frame file {}", path.display()))?;
    parse_rpc_frame(&raw)
}

pub fn dispatch_rpc_frame(frame: &RpcFrame) -> Result<RpcResponseFrame> {
    match frame.kind {
...
#[cfg(test)]
pub fn dispatch_rpc_frame_file(path: &Path) -> Result<RpcResponseFrame> {
    let frame = validate_rpc_frame_file(path)?;
    dispatch_rpc_frame(&frame)
}

pub fn dispatch_rpc_raw_with_error_envelope(raw: &str) -> RpcResponseFrame {
...
pub fn dispatch_rpc_ndjson_input(raw: &str) -> RpcNdjsonDispatchReport {
...
pub fn serve_rpc_ndjson_reader<R, W>(mut reader: R, writer: &mut W) -> Result<RpcNdjsonServeReport>
where
    R: BufRead,
    W: Write,
{
