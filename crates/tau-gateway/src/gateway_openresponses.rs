...
async fn handle_gateway_safety_rules_get(
    State(state): State<Arc<GatewayOpenResponsesServerState>>,
    headers: HeaderMap,
) -> Response {
    if let Err(error) = authorize_and_enforce_gateway_limits(&state, &headers) {
...
async fn handle_gateway_safety_rules_put(
    State(state): State<Arc<GatewayOpenResponsesServerState>>,
    headers: HeaderMap,
    body: Bytes,
) -> Response {
...
async fn handle_gateway_safety_test(
    State(state): State<Arc<GatewayOpenResponsesServerState>>,
    headers: HeaderMap,
    body: Bytes,
) -> Response {
...
fn gateway_safety_rules_path(state_dir: &Path) -> PathBuf {
...
fn read_gateway_safety_rules(path: &Path) -> Result<Option<SafetyRuleSet>, OpenResponsesApiError> {
    if !path.exists() {
        return Ok(None);
    }
    let raw = std::fs::read_to_string(path).map_err(|error| {
